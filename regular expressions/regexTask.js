function demo(input) {
    ...
    let pattern =
        /#(?<type>[A-Za-z ]+)#(?<expiration>\d{2}\/\d{2}\/\d{2})#(?<calories>\d+)#|\|(\k<type>[A-Za-z ]+)\|(\k<expiration>\d{2}\/\d{2}\/\d{2})\|(\k<calories>\d+)\|/g;
    ...
    if (result !== null) {
        for (let entry of result) {
            ...
            if (entry.includes("#")) {
                ...
                console.log(
                    `Item: ${item}, Best before: ${expiration}, Nutrition: ${calories}`
                );
